...
async function generateProfileOG(handle: string) {
  ...
  const CELL_SIZE = 19;
  const CELL_GAP = 2;
  const AVATAR_SIZE = Math.floor(HEIGHT / 3);
  ...
  return new ImageResponse(
    (
      ...
      >
        {/* Header with profile info and avatar */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          {/* Left: Logo and profile info */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 80 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <div style={{ display: 'flex', fontSize: 60, fontWeight: 'bold', color: '#fff' }}>
                  {profile.displayName || profile.handle}
                </div>
                <div style={{ display: 'flex', fontSize: 42, color: '#888' }}>
                  @{profile.handle}
                </div>
              </div>
            </div>
        ...
        {/* Footer - Collection Stats */}
        {hasData && (
          <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', marginTop: 20 }}>
            {collectionStats.stats.slice(0, 6).map((stat, index) => (
              <div key={index} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  />
                  <div style={{ display: 'flex', fontSize: 40, fontWeight: 'bold', color: '#fff' }}>
                    {stat.count.toLocaleString()}
                  </div>
                </div>
                <div style={{ display: 'flex', fontSize: 32, color: '#888' }}>
                  {stat.displayName}
                </div>
              </div>
            ))}
